...
import { getUserByUsername, getUserById, updateUser } from "../db/_db/usersDB.js";
import {
    createRefreshToken,
    getRefreshToken,
    deleteRefreshToken,
...
export async function login(username: string, password: string): Promise<{
    ...
    if (normalizedUsername === process.env.ADMIN_USER?.toLowerCase()) {
        if (password !== process.env.ADMIN_PASS) return null;
        ...
        return {
            accessToken,
            refreshToken,
            user: { userId: "admin", username: normalizedUsername, role: "admin", mustChangePassword: false, createdAt: "" },
        };
    }
